...
public:
    ...
    bool visit_AVL(BinaryTreeNode<ElemType>*root_)
    {
        int lheight;
        int rheight;
        int flag;
        ...
        {
            cout<<root_->data<<" ";
            lheight=this->cal_height(root_->LChild);
            rheight=this->cal_height(root_->RChild);
    ...
    bool check_AVL(BinaryTreeNode<ElemType>*root_)
    {
       if(root_==NULL)
       {
           return false;
       ...
       if(tmp)
       {
           this->check_AVL(tmp->LChild);
           this->check_AVL(tmp->RChild);
           if(!visit_AVL(tmp))
           {
               cout<<"false";
               exit(0);
...
//建立二叉树的存储结构 (外壳）
template<class ElemType>
vector<ElemType> CreateTree(BinaryTree<ElemType> &T, ElemType &str, ElemType &empty){
    ElemType tmp;
    vector<ElemType> t;
    vector<ElemType>res;
    stringstream input_T(str);
...
int main()
{
    ...
    vector<string>res1= CreateTree(trees,tree,gap);
    vector<string>res2=trees.InOrderTraverse(trees.GetRoot());
    sort(res1.begin(),res1.end());
//    print_(res2);
    if(res1==res2)
    {
        if(trees.check_AVL(trees.GetRoot())||trees.GetRoot()==NULL)
        {
            cout<<"true";
        }
        else
