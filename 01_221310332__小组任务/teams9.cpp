...
public:
    ...
    //返回二叉树结点的个数
    void Create_tree_with_level(ElemType &str,ElemType&empty)
    {
        ElemType tmp;
        vector<ElemType>t;
...
int main()
{
 ...
 if(num==0)
 {
     ...
     getline(cin,s1);
     getline(cin,s2);
     stringstream treeq(s1);
     stringstream treez(s2);
     vector<string> treeq_,treez_;
     string c;
     while(treeq>>c)
     {
         treeq_.push_back(c);
 ...
 else
 {
     ...
     string s1,s2;
        getline(cin,s1);
        getline(cin,s2);
     stringstream treez(s1);
     stringstream treeh(s2);
     vector<string> treeh_,treez_;
     string c;
     while(treez>>c)
     {
         treez_.push_back(c);
