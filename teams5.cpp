...
public:
    ...
    void prim(int u) {
       ...
       map<V,pair<V,int>>res;//存放结果
        ...
        for(typename std::map<V,pair<V,int> >::iterator it =res.begin();it!=res.end();it++)
        {
            if(it==res.begin())
            {
    ...
    void Dijkstra(int u)
    {
        ...
        cout<<endl;
        cout<<endl;
        vector<int>find(Vers,0);//初始化find数组
        find[u]=1;
        int k=0;
        ...
        map<int,pair<deque<int>,E>> result;
        for(int i=0;i<Vers;i++)
        {
            deque<int>paths;
            int s=i;
            ...
            result[i].first=paths;
            result[i].second=dist[i];
        }
        for(int i=0;i<dist.size();i++)
        {
            cout<<dist[i]<<" ";
        ...
        cout<<endl;
        cout<<endl;
        for(typename std::map<int,pair<deque<int>,E>>::iterator it=result.begin();it!=result.end();it++)
        {
            if(it->second.first.empty()){ continue;}
            cout<<"<";
            cout<<"("<<it->second.first[0]<<","<<ver[it->second.first[0]]<<")";
            for(int j=1;j<it->second.first.size();j++)
            {
                cout<<",("<<it->second.first[j]<<","<<ver[it->second.first[j]]<<")";
            }
            cout<<">,";
...
void solve_teams_3()
{
    adjmartix_graph<V,int>res1=make_adjmartix_graph<V,int>();
    int u;cin>>u;
    cout<<res1.get_graph_type()<<endl;
    res1.print_vers();
    cout<<endl;
...
template <class V,class E>
void solve_oj_tems_kruskal()
{
    adjlist_graph<V,E>res= make_weight_adjlist_graph<V,E>();
    cout<<res.get_graph_type()<<endl;
    for(int i=0;i<res.kru.size();i++)
    {
        cout<<res.kru[i].weight<<" ";
    }
    cout<<endl;
    for(int i=0;i<res.kru.size();i++)
    {
    ...
    cout<<endl;
    cout<<endl;
    sort(res.kru.begin(),res.kru.end());
    for(int i=0;i<res.kru.size();i++)
    {
        cout<<res.kru[i].weight<<" ";
    }
    ...
    for(int i=0;i<res.kru.size();i++)
    {
        cout<<res.kru[i].index<<" ";
    }
    cout<<endl;
    cout<<endl;
    pair<vector<Kruskal_struct<V,E>>,E> result=res.Kruskal();
    for(int i=0;i<result.first.size();i++)
    {
        cout<<"("<<result.first[i].start<<","<<result.first[i].end<<"),"<<result.first[i].weight<<endl;
    }
    cout<<endl;
    cout<<result.second;
}
